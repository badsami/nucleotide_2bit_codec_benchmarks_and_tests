...
void program_start(void)
{
  const EncodeFunction functions[] =
  {
    ...
    {.name = "SWAR     ", .ptr = encode_bases_bmi2},
  ...
  const u8 expected_bases[] =
  {
    ...
    0b01010000, 0b10101111, 0b11111010, 0b00000101,
    0b01010000, 0b10101111, 0b11111010, 0b00000101,
    0b11110100, 0b11101010, 0b00010111, 0b01000000,
    0b10101001, 0b01111110, 0b00000001
  };

  #define UNENCODED_BASE_COUNT (sizeof(unencoded_bases) - 1)
  #define ENCODED_BYTE_COUNT ((UNENCODED_BASE_COUNT + 3) / 4)
  u8 encoded_bases[ENCODED_BYTE_COUNT];
  ...
  for (u64 func_idx = 0; func_idx < func_count; func_idx += 1)
  {
    ...
    functions[func_idx].ptr(unencoded_bases, encoded_bases, UNENCODED_BASE_COUNT);

    // 4 bases per byte => 32 bases per 8 bytes
    const u8* const encoded_bases_end     = encoded_bases + ENCODED_BYTE_COUNT;
    const u8* const encoded_bases_x32_end = encoded_bases + (ENCODED_BYTE_COUNT & ~7);
    const u8* encoded_bases_ptr  = encoded_bases;
    const u8* expected_bases_ptr = expected_bases;
    while (encoded_bases_ptr < encoded_bases_x32_end)
    {
      u64 encoded_bases_x32 = *(u64*)encoded_bases_ptr;
      u64 expected_bases_x32 = *(u64*)expected_bases_ptr;
      if (encoded_bases_x32 != expected_bases_x32)
      {
        const u64 byte_idx = encoded_bases_ptr - encoded_bases;
        log_literal_str("Failed somewhere between bytes ");
        log_dec_u64(byte_idx);
        log_literal_str(" and ");
        log_dec_u64(byte_idx + 8);
        log_literal_str("\n- expected: 0b");
        log_sized_bin_u64(expected_bases_x32, 64);
        log_literal_str("\n- actual:   0b");
        log_sized_bin_u64(encoded_bases_x32, 64);
        log_character('\n');
        logs_flush();
        goto next;
      }
    ...
    while (encoded_bases_ptr < encoded_bases_end)
    {
      u8 encoded_bases_x4  = *(u8*)encoded_bases_ptr;
      u8 expected_bases_x4 = *(u8*)expected_bases_ptr;
      if (encoded_bases_x4 != expected_bases_x4)
      {
        const u64 byte_idx = encoded_bases_ptr - encoded_bases;
        log_literal_str("Failed at byte ");
        log_dec_u64(byte_idx);
        log_literal_str("\n- expected: 0b");
        log_sized_bin_u64(expected_bases_x4, 8);
        log_literal_str("\n- actual:   0b");
        log_sized_bin_u64(encoded_bases_x4, 8);
        log_character('\n');
        logs_flush();
        goto next;
      }
